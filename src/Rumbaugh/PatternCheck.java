...
public static void floodFill(int x, int y, int[][] array) {
	...
	while(!queue.isEmpty()){
		s = queue.remove();
		if(s.startsWith("0") || s.startsWith("3") || s.startsWith("9")){
			...
			boolean b= true;
			while(w>0 && b){
				if(array[x][w-1] == 0 || array[x][w-1] == 3 || array[x][w-1] == 9)
					w--;
				else
					b=false;
			}
			b= true;
				while(e<RobotData.ARRAY_LENGTH-1 && b)
					if(array[x][e+1] == 0 || array[x][e+1] == 3|| array[x][e+1] == 9)
						e++;
					else 
						b= false;
			...
			for(int j= w;j<=e;j++){
				array[x][j]= 8;
				if(x>0)
					if(array[x-1][j] == 0 || array[x-1][j] == 3 || array[x-1][j] == 9)
